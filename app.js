...
const App = (() => {
  ...
  function resetWeek() {
    if (!confirm('�Reiniciar semana? Se borrar�n todos los pesos, reps y estados de esta semana.')) return;
    const keys = [
      storageKey('sets'),
      storageKey('notes'),
      storageKey('exdone'),
  ...
  function completeDay(dayIdx) {
    ...
    updateWeekProgress();
    if (data[dayIdx]) showToast('?? �D�a completado! Buen trabajo');
  }
  ...
  function renderDayStatus(dayIdx, done) {
    ...
    if (span) span.textContent = done ? '? D�a completado' : 'Marcar d�a completado ?';
  ...
  function updateWeekProgress() {
    ...
    if (text) text.textContent = `${completed}/5 d�as`;
  }
